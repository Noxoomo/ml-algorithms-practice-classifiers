...
public class DataFrame {
    ...
    public boolean canSplit(int feature, int[] index) {

        for (int i = 1; i < index.length; ++i)
            if (data[feature][index[i]] != data[feature][index[i - 1]])
                return true;
        return false;
    }

    public SplitPoint findSplit(int feature, int[] index) {
        //initialize
        int[] order = argsort(feature, index);

        int currentSplit = 1;
        while (currentSplit < index.length && data[feature][order[currentSplit - 1]] == data[feature][order[currentSplit]])
            currentSplit++;
        ...
        while (currentSplit < index.length) {
            do {
